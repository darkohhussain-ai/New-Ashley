...
export default function CashWithdrawalDashboardPage() {
  ...
  const monthlyData = useMemo(() => {
    ...
    filteredRecords.forEach(record => {
      employeeTotals.set(record.employeeId, (employeeTotals.get(record.employeeId) || 0) + record.amount);
    });
    ...
    return { 
      ...
      chartData: summary.map(s => ({ name: s.employeeName, [t('total_withdrawn')]: s.totalAmount }))
    };
  }, [isLoading, selectedDate, withdrawals, employees, t]);

  const menuItems = [
    {
      title: t('add_daily_withdrawal'),
      icon: Plus,
      href: "/cash-withdrawal/add",
      color: "bg-blue-500",
    },
    {
      title: t('monthly_withdrawal_report'),
      icon: Calendar,
      href: "/cash-withdrawal/monthly-report",
      color: "bg-orange-500",
    },
    {
      title: t('withdrawal_archive'),
      icon: Archive,
      href: "/cash-withdrawal/archive",
      color: "bg-teal-500",
    },
  ];
