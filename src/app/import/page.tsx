...
import { ArrowLeft, Upload, File, User, Calendar as CalendarIcon, Building, Loader2, CheckCircle, AlertTriangle, ChevronRight, Save } from 'lucide-react';
...
export default function ImportPage() {
  ...
  const handleNextStep = async () => {
    if (!file || !storekeeperId || !source || !date) {
    ...
    reader.onload = (e) => {
      try {
        ...
        if (parsedItems.length === 0) {
          toast({ variant: 'destructive', title: 'No Data Found', description: 'The Excel file seems to be empty or not formatted correctly. Ensure it has "Model" and "Quantity" columns.' });
        ...
        <div>
            <Card className="mb-8">
                <CardHeader>
                    ...
                    <CardDescription>
                        File: <span className="font-semibold text-primary">{fileName}</span> | 
                        Storekeeper: <span className="font-semibold text-primary">{employees?.find(e => e.id === storekeeperId)?.name}</span> | 
                        Date: <span className="font-semibold text-primary">{date ? format(date, 'PPP') : ''}</span>
                    </CardDescription>
                </CardHeader>
            </Card>
            <Card>
