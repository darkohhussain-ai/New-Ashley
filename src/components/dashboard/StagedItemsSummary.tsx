  ...
  return (
    <Card className="border-none shadow-sm overflow-hidden">
      <CardHeader className="bg-muted/30 py-3 px-4">
        <CardTitle className="text-base">{t('staged_items_summary')}</CardTitle>
        <CardDescription className="text-[11px]">{t('staged_items_summary_desc')}</CardDescription>
      </CardHeader>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            {destinationStats.map(stat => (
                <div 
                  ...
                  className="p-3 flex flex-col justify-between hover:bg-muted/50 cursor-pointer transition-all rounded-xl border-2 hover:shadow-sm" 
                  style={{ borderColor: stat.color + '20'}} 
                  onClick={() => router.push(`/transmit/staged?destination=${stat.destination}`)}
                >
                    <div className="space-y-0.5">
                        <p className="font-bold text-xs flex items-center gap-1.5">
                            <MapPin className="w-3 h-3" style={{ color: stat.color }} />
                            {stat.destination}
                        </p>
                        <p className="text-xl font-black">
                            {stat.stagedItemCount} 
                            <span className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground ml-1.5">{t('items_lowercase')}</span>
                        </p>
                    </div>
                    <div className="text-[9px] text-muted-foreground mt-3 pt-1.5 border-t uppercase font-black tracking-tighter opacity-60">
